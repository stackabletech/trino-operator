...
Different authentication types can be used in parallel.
...
[source,yaml]
----
include::example$usage_guide/trino-file-auth-snippet.yaml[tag=authentication_class]
----
...
For uniqueness, the volume mount name is internally build up of the name of the `AuthenticationClass` and the name of the referenced `Secret` concatenated with a "-" character (e.g. "simple-trino-file-auth-simple-trino-users" for the example above). This means the `AuthenticationClass` and `Secret` names combined must be smaller than *62* characters.

IMPORTANT: Due to Kubernetes restrictions, the concatenated names of the AuthenticationClass and its referenced Secret must not exceed *62* characters
...
[source,yaml]
----
include::example$usage_guide/trino-ldap-auth-snippet.yaml[tag=authentication_class]
----
