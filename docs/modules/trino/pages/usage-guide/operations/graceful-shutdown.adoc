...
When you are using the rego rules provided by Stackable this permission is automatically granted.
However, when you are writing your own rego rules, you can achieve this e.g. by adding the following rule, which grants `graceful-shutdown-user` the permissions to issue a graceful shutdown.
