...
Create an insecure single node Trino (v387) cluster for testing. This can be accessed with the UI/CLI via http without either user/password credentials or authorization (although `minio-credentials` should be provided for `s3` access):
...
The following steps must be taken to secure a Trino cluster:
