...
In the simplest form, Trino does not require any other operators (except the commons and secret operator) to work and access the web interface. There are no data sources (e.g. PostgreSQL, Hive, S3 ...) connected and for the tests only internal data is queried.
...
We use the https://trino.io/download.html[Trino CLI tool] to access the Trino cluster. This link points to the latest Trino version. In this guide we keep cluster and client versions (v387) in sync and download the CLI tool from the https://repo.stackable.tech/[Stackable repository]:
