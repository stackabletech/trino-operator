...
After going through the xref:installation.adoc[] section and having installed all the operators, you will now deploy a Trino cluster and the required dependencies. Afterwards you can <<_verify_that_it_works, verify that it works>> by running some queries against Trino or visit the Trino web interface.

== Setup Trino

In the simplest form, Trino does not require any other operators (except the commons and secret operator) to work and access the web interface. There are no data sources (e.g. PostgreSQL, Hive or S3) configured and for the tests only internal data is queried.
...
We use the https://trino.io/download.html[Trino CLI tool] to access the Trino cluster. This link points to the latest Trino version. In this guide we keep Trino cluster and client versions (v387) in sync and download the CLI tool from the https://repo.stackable.tech/[Stackable repository]:
...
In order to remove the previous downloaded CLI tool, you can safely remove it from your system:

[source]
----
include::example$code/getting-started.sh[tag=cleanup-trino-cli]
----
