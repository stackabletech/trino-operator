...
You then can have two catalogs called e.g. `warehouse-1` and `warehouse-2` that both use the `hive` connector.
...
This mechanism allows to create a `TrinoCluster` once and than add new catalogs dynamically by simple creating `TrinoCatalog` objects.
