...
pub struct TrinoConfig {
    ...
    /// The current solution only appends the JVM arguments to the ones the operator generated. The
    /// future implementation will probably allow you to override arguments generated by the
    /// operator (similar to how configOverrides and podOverrides are working).
    //
...
mod tests {
    ...
    #[test]
    fn test_experimental_jvm_arguments() {
        let input = r#"
        apiVersion: trino.stackable.tech/v1alpha1
        kind: TrinoCluster
        metadata:
        ...
        assert_eq!(
            trino
                ...
                .config
                .config
                .experimental_additional_jvm_arguments,
            BTreeMap::from([
                ("-XX:+ExitOnOutOfMemoryError".to_owned(), JvmArgument(None)),
                (
                ...
                (
                    ...
                    JvmArgument(Some("512M".to_owned()))
                )
            ])
        );
    }
