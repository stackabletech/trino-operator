...
///
/// Returns an empty map when client spooling is not configured and no user overrides are provided.
...
pub fn build(cluster: &ValidatedCluster, rg: &TrinoRoleGroupConfig) -> BTreeMap<String, String> {
    ...
    props.extend(rg.config_overrides.spooling_manager_properties.clone());
